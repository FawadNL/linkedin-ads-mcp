...
export class LinkedInOAuth {
  ...
  async authenticate(): Promise<LinkedInTokens> {
    return new Promise((resolve, reject) => {
      ...
      // Declare timeout before app.listen — Express 5 fires the error
      // callback synchronously for immediate errors like EADDRINUSE.
  ...
  private async exchangeCodeForTokens(code: string): Promise<LinkedInTokens> {
    ...
    const tokens = (await response.json()) as Omit<
      LinkedInTokens,
      "expires_at"
    >;
